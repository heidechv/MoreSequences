...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(40.0, 30.0),  # All numbers are approximate.
                rg.Point(30.0, 40.0),
                rg.Point(20.0, 30.0),
                rg.Point(30.0, 20.0)]
    circle = rg.Circle(rg.Point(30, 30), 10)
...
def run_test_draw_points_on_circle():
    ...
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(60, 100), 30)
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 4:  500 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
...
def run_test_polygon():
    ...
    circle.outline_thickness = 5
    polygon(window, circle, 8, 'green', 2)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(vertices)):
        if k + 1 < len(vertices):
            side = rg.Line(vertices[k], vertices[k+1])
        else:
            side = rg.Line(vertices[k], vertices[0])
        side.color = color
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON test 4:  16 midnight blue lines on orange circle, hops = 10.'
    window = rg.RoseWindow(500, 500, title)

    circle = rg.Circle(rg.Point(250, 250), 225)
    circle.fill_color = 'orange'
    fancy_polygon(window, circle, 16, 10, 'midnight blue', 10)
    window.close_on_mouse_click()
